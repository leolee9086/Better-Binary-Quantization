...
describe('暴力查询性能测试', () => {
    ...
    DIMENSIONS.forEach(dim => {
      BASE_SIZES.forEach(baseSize => {
        it(`${dim}维向�?${baseSize}个向量的暴力查询性能`, () => {
          ...
          console.log(`  计算复杂�? O(${baseSize} × ${dim})`);
  ...
  describe('量化索引构建时间测试', () => {
    it('测试量化索引构建时间', () => {
      ...
      console.log(`每向量构建时�? ${(buildTime.avgTime / baseSize).toFixed(3)}ms`);
  ...
  describe('暴力查询与量化查询性能对比', () => {
    it('1024维向量暴力查询与量化查询对比', () => {
      ...
      const queryVectors = generateVectors(QUERY_SIZE, dim);
             ...
             // 构建量化索引（不计入查询时间�?       const format = new BinaryQuantizationFormat({
         queryBits: 1,
         indexBits: 1,
         quantizer: {
           similarityFunction: VectorSimilarityFunction.COSINE,
      ...
      // 测试暴力查询（纯查询时间�?      const bruteForceMethod = (): SearchResult[][] => {
      ...
      // 测试量化查询（纯查询时间，不包含构建时间�?      const quantizedMethod = () => {
        const results = [];
        for (let i = 0; i < QUERY_SIZE; i++) {
          const query = queryVectors[i]!;
          const quantizedResults = format.searchNearestNeighbors(query, quantizedVectors, K);
          // 转换为与暴力查询相同的结�?          const convertedResults = quantizedResults.map(r => ({
            index: r.index,
            similarity: r.score
          }));
      ...
      // 验证结果一致�?      const bruteForceResults = bruteForceMethod();
      ...
      for (let i = 0; i < QUERY_SIZE; i++) {
        ...
        if (intersection.size >= K * 0.8) { // 80%一致�?          consistencyCount++;
        }
      }
      
      console.log(`结果一致�? ${consistencyCount}/${QUERY_SIZE} 个查询达�?0%一致性`);
      
      // 性能断言（允许量化查询在小规模数据上可能更慢�?      expect(speedup).toBeGreaterThan(0.1); // 量化查询不应比暴力查询慢10倍以�?      expect(quantizedTime.avgTime).toBeLessThan(bruteForceTime.avgTime * 10); // 量化查询时间不应超过暴力查询10�?    });
  });
  ...
  describe('大规模数据暴力查询性能', () => {
    it('大规模数据的暴力查询性能测试', () => {
      ...
      largeSizes.forEach(baseSize => {
        ...
        // 生成大规模数�?        const vectors = generateVectors(baseSize, dim);
        const queryVectors = generateVectors(3, dim); // 只测�?个查询以节省时间
        
        // 测试单个查询的性能
        const query = queryVectors[0]!;
        const { avgTime } = measurePerformance(
          `暴力查询 ${baseSize}�?{dim}维向量`,
          () => bruteForceSearch(query, vectors, K),
          1
        );
        
        // 计算理论复杂�?        const operations = baseSize * dim * 2; // 每个向量需要dim次乘法和dim次加�?        const operationsPerMs = operations / avgTime;
        
        console.log(`  理论操作�? ${operations.toLocaleString()}`);
        console.log(`  操作速度: ${Math.round(operationsPerMs).toLocaleString()} 操作/ms`);
        console.log(`  预计100万向量查询时�? ${((1000000 * dim * 2) / operationsPerMs / 1000).toFixed(1)}秒`);
  ...
  describe('内存使用对比', () => {
    it('暴力查询与量化查询的内存使用对比', () => {
      ...
      const { quantizedVectors } = format.quantizeVectors(vectors);
      const quantizedMemory = quantizedVectors.size() * (dim / 8); // 1位量化：�?个维�?字节
      
      console.log(`\n📊 内存使用对比:`);
      console.log(`原始向量内存: ${(originalMemory / 1024 / 1024).toFixed(2)} MB`);
      console.log(`量化向量内存: ${(quantizedMemory / 1024 / 1024).toFixed(2)} MB`);
      console.log(`内存压缩�? ${(originalMemory / quantizedMemory).toFixed(1)}:1`);
      console.log(`内存节省: ${((1 - quantizedMemory / originalMemory) * 100).toFixed(1)}%`);
      
      // 内存使用断言
      expect(quantizedMemory).toBeLessThanOrEqual(originalMemory / 8); // 至少8倍压�?    });
  });
}); 
